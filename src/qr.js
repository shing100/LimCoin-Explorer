 ...
 *
 * 왜 직접 쓰는가: 주소를 휴대폰으로 옮기는 가장 확실한 방법이 QR 인데,
 * 이걸 위해 라이브러리를 하나 더 매달고 싶지 않았다. 더 중요한 이유는
 * **주소를 바깥으로 보내지 않기 위해서**다. QR 이미지를 만들어 주는 웹
 * 서비스에 주소를 넘기면, 그 서비스는 "누가 어느 주소를 조회했는지"를
 * 그대로 알게 된다. 익명성이 얼마 없는 체인에서 이건 공짜로 주는 정보다.
 *
